	...
	on( 'submit', '.js-form-profile', function ( event ) {
		...
		ajax( {
			...
			if ( result.hasOwnProperty( 'success' ) && true === result[ 'success' ] ) {
				let qwe = form.querySelector( '.js-form__info' ).innerHTML = 'Данные сохранены';
				qwe.setTimeout(explode, 2000);
              //  setTimeout{getElementsByClassName('.box')},3000);  //3000 = 3 секунд
			}
